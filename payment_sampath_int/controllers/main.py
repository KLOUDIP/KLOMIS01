...
class SampathIntController(http.Controller):
    _return_url = '/payment/sampath_int/return'
    _webhook_url = '/payment/sampath_int/webhook'

    def _confirm_sampath_int_transaction(self, payment_response):
        """
        @private - confirm the transaction with response data
        """
        provider_id = request.env['payment.provider'].with_user(SUPERUSER_ID).search([('code', '=', 'sampath_int')], limit=1)
        tx_sudo = request.env['payment.transaction'].sudo()._get_tx_from_notification_data('sampath_int', payment_response)
    ...
    def sampath_int_return_from_checkout(self, **raw_data):
        ...
        """
        tx_sudo = request.env['payment.transaction'].sudo()._get_tx_from_notification_data('sampath_int', raw_data)
        _logger.info("handling redirection from Sampath with data:\n%s", pprint.pformat(raw_data))

        confirm_response = self._confirm_sampath_int_transaction(raw_data)
        _logger.info("Processing transaction with data:\n%s", pprint.pformat(confirm_response))

        tx_sudo._handle_notification_data('sampath_int', confirm_response)
