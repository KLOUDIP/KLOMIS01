...
class ProjectTaskLine(models.Model):
    ...
    def _compute_is_tech_team_user(self):
        for rec in self:
            if rec.env.user.has_group('base.group_system') or self.env.user.id == rec.tech_team_member_id.id:
                rec.is_tech_team_user = True
            else:
