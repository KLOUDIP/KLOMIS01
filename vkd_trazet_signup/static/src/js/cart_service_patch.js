...
patch(CartService.prototype, {
     ...
     * payload when a Trazet product was blocked from being added because the
     * visitor doesn't have a Trazet account yet (see controllers/shop.py:
     * TrazetCartGate.add_to_cart). Checked right after the RPC call, before
     * the "Buy Now" branch below, since that branch redirects to /shop/cart
     * on its own and never reaches `_showCartNotification`.
     */
    async _makeRequest({
        productTemplateId,
        productId,
