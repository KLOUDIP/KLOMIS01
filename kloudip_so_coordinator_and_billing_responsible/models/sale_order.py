...
class SaleOrder(models.Model):
    ...
    coordination_by_id = fields.Many2one(
        'hr.employee',
        string='Coordination By',
        compute='_compute_coordination_and_billing_responsible',
        store=True,
        readonly=False,
        precompute=True
    )
    ...
    @api.depends('partner_id', 'partner_id.coordination_by_id', 'partner_id.billing_by_id',
                 'partner_id.parent_id.coordination_by_id', 'partner_id.parent_id.billing_by_id')
    def _compute_coordination_and_billing_responsible(self):
        """ Get coordination and billing to current sale order """
        for rec in self:
            if rec.partner_id:
                if rec.partner_id.parent_id:
                    rec.coordination_by_id = rec.partner_id.parent_id.coordination_by_id.id
                    rec.billing_by_id = rec.partner_id.parent_id.billing_by_id.id
                else:
                    rec.coordination_by_id = rec.partner_id.coordination_by_id.id
                    rec.billing_by_id = rec.partner_id.billing_by_id.id
            else:
                rec.coordination_by_id = False
