 ...
 * Trazet sign-up module is installed too its wrapper nests with this one, and
...
patch(CartService.prototype, {
        ...
        this.rpc = async (route, params, settings) => {
            ...
            try {
                ...
                if (GATED_ROUTES.has(route)) {
                    this._fiosReportCartFailure(route, error);
                }
                throw error;
            }
     ...
     */
    _fiosReportCartFailure(route, error) {
        if (error?.[REPORTED]) {
            return;
        }
        try {
